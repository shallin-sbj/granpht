...
public class DijkstraSolve2 {

    private final String sourceNode;

    private final MutableValueGraph<String, Integer> graph;

    public DijkstraSolve2(String sourceNode, MutableValueGraph<String, Integer> graph) {
    ...
    public static void main(String[] args) {
        int num = 100;
        int count = 1000;
        MutableValueGraph<String, Integer> graph = buildGraph(num, count);
        Set<String> nodes = graph.asGraph().nodes();
        System.out.println("nodes :"+ nodes.size());

//        DijkstraSolve2 dijkstraSolve = new DijkstraSolve2("v4", graph);
//
...
//        dijkstraSolve.printResult();
     ...
     * @return
     */
    private static MutableValueGraph<String, Integer> buildGraph(int num, int count) {
        ...
        for (int i = 0; i < count; i++) {
            int nextInt = getNextInt(num);
            graph.putEdgeValue(getNextInt(num)+"",getNextInt(num)+"", nextInt);
        }
...
//        graph.putEdgeValue("D", "E", 11);
    ...
    private static int getNextInt(int num) {
        return new Random(num).nextInt();
    }
}
